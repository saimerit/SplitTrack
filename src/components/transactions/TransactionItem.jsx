...
const TransactionItem = ({ txn, linkedRefunds = [], participantsLookup, onEdit, onDelete, selectionMode, isSelected, onToggleSelect, onClone, index = 0 }) => {
  // Mobile Modal State
  ...
  return (
    <>
      ...
      <div
        ...
        className={`sm:hidden p-3 mb-3 glass-card flex flex-row justify-between items-center transition-all cursor-pointer group animate-enter-card ${isSelected ? 'bg-indigo-50/10 border-indigo-500/30' : ''}`}
        style={{ animationDelay: `${index * 50}ms` }}
      >
        <div className="flex gap-3 items-center min-w-0 flex-1">
          {selectionMode && (
            <div className="shrink-0 mr-2 text-indigo-400">
              {isSelected ? <CheckCircle size={22} className="fill-indigo-100 dark:fill-indigo-900" /> : <Circle size={22} className="text-gray-400" />}
            </div>
          )}
          <div className="shrink-0">{getCategoryIcon(txn.category)}</div>
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-gray-100 truncate pr-2">{txn.expenseName}</p>
        ...
        <div className="text-right shrink-0 ml-2 flex flex-col items-end">
          <p className="font-bold text-gray-200">{formatCurrency(amount)}</p>
          {shareText && <p className={`text-[10px] ${shareColor}`}>{shareText}</p>}
        </div>
        <ChevronRight size={16} className="text-gray-600 ml-2 shrink-0" />
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-60 flex items-end sm:items-center justify-center sm:p-6 animate-fade-in sm:hidden">
          ...
          <div className="relative w-full h-[95vh] bg-gray-900 text-gray-100 rounded-t-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up border border-gray-800">

            {/* Modal Header */}
            <div className="p-5 border-b border-gray-800 flex justify-between items-start bg-gray-800/50 shrink-0">
              <div className="pr-4">
                <h3 className="text-xl font-bold text-white leading-snug wrap-break-word">{txn.expenseName}</h3>
                <p className="text-sm text-gray-400 mt-1 flex items-center gap-2"><Calendar size={14} /> {formatDate(txn.timestamp)}</p>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 bg-gray-800 rounded-full hover:bg-gray-700 transition-colors shrink-0"><X size={20} className="text-gray-400" /></button>
            </div>

            {/* Modal Body */}
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              <div className="flex items-center justify-between p-4 bg-sky-900/20 rounded-xl border border-sky-800">
                <span className="text-sm font-medium text-sky-300">Total Amount</span>
                <span className="text-3xl font-bold text-sky-400">{formatCurrency(txn.amount)}</span>
              </div>

              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="p-3 bg-gray-800/50 rounded-lg">
                  <span className="text-xs text-gray-500 uppercase font-bold block mb-1">Paid By</span>
                  <span className="font-medium text-gray-200">{payerName}</span>
                </div>
                <div className="p-3 bg-gray-800/50 rounded-lg">
                  <span className="text-xs text-gray-500 uppercase font-bold block mb-1">Category</span>
                  <div className="flex items-center gap-2">{getCategoryIcon(txn.category)}<span className="font-medium text-gray-200">{txn.category || 'Uncategorized'}</span></div>
                </div>
                {txn.place && <div className="p-3 bg-gray-800/50 rounded-lg col-span-2 flex items-center gap-2"><MapPin size={16} className="text-gray-400" /><span className="font-medium text-gray-200">{txn.place}</span></div>}
                ...
                <div>
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-lg divide-y divide-gray-800">
                    {Object.entries(txn.splits).map(([uid, val]) => {
                      ...
                      return (<div key={uid} className="flex justify-between items-center p-3 text-sm"><span className="text-gray-300 font-medium">{getName(uid)}</span><span className="font-mono text-gray-200">{formatCurrency(val)}</span></div>);
                    })}
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {linkedRefunds.map(child => (
                      <div key={child.id} className="flex items-center justify-between p-3 bg-indigo-900/20 rounded-lg border border-indigo-800 text-sm">
                        <div className="flex items-center gap-2 text-indigo-300"><RefreshCw size={14} /><span>{child.isReturn ? 'Settlement' : 'Refund'}</span></div>
                        <span className="font-bold text-indigo-300">{formatCurrency(Math.abs(child.amount))}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {/* Glass Card Style */}
      <div
        className={`hidden sm:flex p-4 mb-3 glass-card flex-row justify-between items-start transition-all group animate-enter-card ${isSelected ? 'bg-indigo-50/10 border-indigo-500/30' : 'hover:-translate-y-0.5'}`}
        style={{ animationDelay: `${index * 50}ms` }}
      >
        <div className="flex gap-3 flex-1 overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-gray-100 truncate">{txn.expenseName}</p>
            ...
            {linkedRefunds.length > 0 && (
              <div className="mt-2 space-y-1">
                {linkedRefunds.map(child => (
                  <div key={child.id} className="flex items-center text-xs text-indigo-400">
            ...
            <button onClick={() => setIsExpanded(!isExpanded)} className="mt-2 text-xs text-sky-400 hover:text-sky-300 hover:underline flex items-center gap-1">
        ...
        <div className="flex flex-col items-end ml-4 text-right shrink-0">
          <div>
            <p className="font-bold text-lg text-gray-200">{formatCurrency(amount)}</p>
