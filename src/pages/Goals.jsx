  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-24">
      <div className="glass-card p-6 md:p-8 flex justify-between items-center">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-linear-to-r from-green-400 to-emerald-400">Savings Goals</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {goals.map(g => {
          ...
          return (
            <div key={g.id} className="glass-card p-6 relative group">
              <div className="flex justify-between mb-2">
                <h3 className="font-bold text-lg text-gray-200">{g.name}</h3>
                <button
              ...
              <div className="w-full bg-white/10 rounded-full h-3 mb-2 overflow-hidden">
                <div className="h-3 rounded-full transition-all duration-1000 ease-out" style={{ width: `${percent}%`, backgroundColor: 'var(--primary)' }}></div>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-gray-500">{formatCurrency(currentSavings * 100)} saved</span>
                <span className="font-bold" style={{ color: 'var(--primary)' }}>{Math.round(percent)}%</span>
              </div>
            </div>
          );
        })}
        ...
        <div
          ...
          className="border-2 border-dashed border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-white/30 hover:bg-white/5 transition"
        >
