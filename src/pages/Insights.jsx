    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto pb-24">
            <div className="glass-card p-6 md:p-8">
                <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">Insights</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {suggestions.length > 0 ? suggestions.map((s, i) => (
                    <div key={i} className="glass-card p-4 flex items-start gap-3">
                        ...
                        <div>
                            <h4 className="font-bold text-emerald-400 text-sm uppercase tracking-wide">{s.title}</h4>
                            <p className="text-gray-300 text-sm mt-1" dangerouslySetInnerHTML={{ __html: s.text }}></p>
                        </div>
                    </div>
                )) : (
            ...
            <h3 className="text-xl font-bold text-gray-200">This Week's Snapshot</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="glass-card p-4">
                    <p className="text-sm text-gray-500">Total Spent</p>
                    <p className="text-2xl font-bold text-gray-100 mt-1">{formatCurrency(weeklyStats.total * 100)}</p>
                </div>
                <div className="glass-card p-4">
                    <p className="text-sm text-gray-500">Daily Average</p>
                    <p className="text-2xl font-bold text-gray-100 mt-1">{formatCurrency(weeklyStats.dailyAvg * 100)}</p>
                </div>
                <div className="glass-card p-4">
            ...
            <div className="glass-card p-6">
                <h4 className="font-semibold text-gray-300 mb-4">Top Categories This Week</h4>
                        ...
                        <div key={cat} className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="w-24 h-2 bg-white/10 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full" style={{ width: `${(val / weeklyStats.total) * 100}%`, backgroundColor: 'var(--primary)' }}></div>
                                </div>
                                <span className="text-xs font-bold text-gray-200">₹{Math.round(val)}</span>
                            </div>
                        </div>
                    ))}
