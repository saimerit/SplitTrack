...
const Login = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-center" style={{ backgroundColor: 'var(--bg-main)' }}>
      <div className="w-full max-w-sm p-8 rounded-lg shadow-xl border border-white/10" style={{ backgroundColor: 'var(--bg-surface)' }}>
        <h2 className="text-3xl font-bold text-center" style={{ color: 'var(--primary)' }}>SplitTrack</h2>
        <p className="text-center text-gray-400 mt-2 mb-8">
          Please sign in to continue.
        </p>

        {errorMsg && (
          <div className="mb-4 p-3 bg-red-900/30 text-red-400 rounded text-sm text-center border border-red-800">
            {errorMsg}
          </div>
        )}
        ...
        <button
          onClick={handleLogin}
          className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-white/10 rounded-lg shadow-sm hover:bg-white/5 transition-colors"
        ...
        >
          ...
          <span className="text-base font-medium text-gray-200">Sign in with Google</span>
        </button>
      </div>
    </div>
  );
