...
const Settings = () => {
    ...
    setTimeout(() => {
      if (palette.id === 'blackout') {
        ...
        if (palette.type !== theme && palette.id !== 'default') {
          ...
          showToast(`Switched to ${palette.type} mode.`);
        }
      }
    }, 50);
  };
  ...
  const handleRemoveMember = async (email) => {
    setModalConfig({
      isOpen: true,
      title: "Remove Member?",
      message: `Remove <strong>${email}</strong>?`,
      confirmInput: "REMOVE",
      confirmText: "Remove",
      onConfirm: async () => {
        try {
  ...
  const handleImportJSON = async (e) => {
    ...
    setModalConfig({
      ...
      message: "Wipes current data.",
      confirmInput: "DELETE ALL",
      confirmText: "Restore",
      onConfirm: async () => {
        setLoading(true);
        try {
          const s = await importFromBackup(file);
          await setDoc(doc(db, 'ledgers/main-ledger'), s, { merge: true });
  ...
  const handleCSVImport = async () => {
    ...
    try {
      ...
      const add = (s, arr, c) => s.forEach(n => {
        if (!arr.find(i => i.name === n)) batch.set(doc(collection(db, `ledgers/main-ledger/${c}`)), { name: n });
      });
      add(newMeta.categories, categories, 'categories');
      add(newMeta.places, places, 'places');
      add(newMeta.tags, tags, 'tags');
      add(newMeta.modes, modesOfPayment, 'modesOfPayment');
      await batch.commit();

      const BATCH_SIZE = 400;
      for (let i = 0; i < txnsToAdd.length; i += BATCH_SIZE) {
        const tb = writeBatch(db);
        txnsToAdd.slice(i, i + BATCH_SIZE).forEach(t => tb.set(doc(collection(db, 'ledgers/main-ledger/transactions')), t));
        await tb.commit();
      }
      showToast(`Imported ${txnsToAdd.length} items!`);
      setCsvFile(null);
    } catch (e) {
  ...
  const handleNuke = async (n) => {
    setModalConfig({
      isOpen: true,
      title: `Delete All ${n}?`,
      message: `Delete ALL ${n}? Undone.`,
      confirmInput: `DELETE ${n.toUpperCase()}`,
      confirmText: "Delete All",
      onConfirm: async () => {
        setLoading(true);
        try {
          await nukeCollection(n);
          showToast("Deleted.");
        } catch {
          showToast("Failed.", true);
        }
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          snap.docs.forEach(d => {
            ...
            c++;
            if (c >= 400) {
              batches.push(cb.commit());
              cb = writeBatch(db);
              c = 0;
            }
          });
          if (c > 0) batches.push(cb.commit());
    ...
    <div className="space-y-6 md:space-y-8 max-w-5xl mx-auto pb-20 animate-fade-in">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow border dark:border-gray-700">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">Appearance</h2>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-sm font-bold text-gray-500 uppercase">Palettes</h3>
            <button onClick={() => setShowCreator(!showCreator)} className="text-xs text-sky-600 font-bold hover:underline">{showCreator ? 'Cancel' : '+ New'}</button>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4 my-4">
                {['bgMain', 'bgSurface', 'primary', 'textMain'].map(key => (
                  <div key={key}>
                    ...
                    <input type="color" value={newColors[key]} onChange={e => setNewColors({ ...newColors, [key]: e.target.value })} className="h-10 w-full p-0 border-0 rounded" />
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-2 mb-6 max-h-40 overflow-y-auto">
            {(userSettings.allowed_emails || []).map(email => (
              <div key={email} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded">
                ...
                {email !== user?.email && <button onClick={() => handleRemoveMember(email)} className="text-red-500 text-xs font-bold shrink-0">REMOVE</button>}
              </div>
            ))}
          </div>
          <form onSubmit={handleAddMember} className="flex gap-2">
            <Input value={newMember} onChange={e => setNewMember(e.target.value)} placeholder="Email" className="flex-1" />
        ...
        <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow border dark:border-gray-700">
          ...
          {healthReport ? (
            <div className="space-y-3">
              <div className={`p-3 rounded border flex items-center gap-2 ${healthReport.issues === 0 ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'}`}>
                {healthReport.issues === 0 ? <CheckCircle size={18} /> : <AlertTriangle size={18} />}
                <span className="text-xs font-medium">{healthReport.issues === 0 ? "Healthy" : `${healthReport.issues} Issues`}</span>
              </div>
              {healthReport.report.length > 0 && (
                <div className="max-h-32 overflow-y-auto text-[10px] font-mono border rounded p-2">
                  {healthReport.report.map((l, i) => (
                    <div key={i} className={l.type === 'error' ? 'text-red-500' : 'text-yellow-600'}>{l.message}</div>
                  ))}
                </div>
              )}
            </div>
          ) : <p className="text-sm text-gray-500">Run diagnostics to check status.</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow border dark:border-gray-700">
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="text-sm text-gray-500 mb-2">Import CSV</p>
              <button onClick={downloadCSVTemplate} className="text-xs text-sky-600 hover:underline mb-2 block">Download Template</button>
              <input type="file" accept=".csv" onChange={e => setCsvFile(e.target.files[0])} className="block w-full text-xs text-gray-500 file:mr-2 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-sky-50 file:text-sky-700" />
              <Button onClick={handleCSVImport} disabled={!csvFile || loading} className="w-full mt-2">Import CSV</Button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow border dark:border-gray-700">
          ...
          <div className="space-y-4">
            <Button onClick={handleExportJSON} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700 flex justify-center gap-2">
              <Database size={16} /> Export Backup
            </Button>
            <hr className="dark:border-gray-700" />
            <div>
              <p className="text-sm text-red-500 mb-2 font-bold">Restore (Dangerous)</p>
              <input type="file" accept=".json" onChange={handleImportJSON} className="block w-full text-xs text-gray-500 file:mr-2 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-red-50 file:text-red-700" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <Button onClick={handleEmptyTrash} variant="danger" size="sm" className="text-xs">
              <Trash2 size={14} className="mr-1 inline" /> Empty
            </Button>
            {!showTrash && <Button onClick={fetchTrash} variant="secondary" size="sm" className="text-xs">View Items</Button>}
          </div>
        </div>
        {showTrash && (
          <div className="space-y-2">
            {trashItems.length === 0 ? <p className="text-gray-500 italic text-center">Empty</p> : trashItems.map(i => (
              <div key={i.id} className="flex justify-between items-center p-2 bg-red-50 dark:bg-red-900/20 rounded border border-red-100">
                <div className="text-xs">
                  <p className="font-bold">{i.expenseName}</p>
                  <p>{formatCurrency(i.amount)}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleRestore(i.id)} className="text-green-600 text-xs font-bold">RESTORE</button>
                  <button onClick={() => handleHardDelete(i.id)} className="text-red-600 text-xs font-bold">DELETE</button>
                </div>
              </div>
            ))}
            <Button onClick={() => setShowTrash(false)} variant="ghost" className="w-full text-xs">Hide</Button>
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow border border-red-200 dark:border-red-900">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          {['Transactions', 'Participants', 'Categories', 'Places', 'Tags', 'Modes'].map(n => (
            <Button key={n} onClick={() => handleNuke(n === 'Modes' ? 'modesOfPayment' : n.toLowerCase())} variant="danger" className="text-xs py-3">{n}</Button>
          ))}
        </div>
      </div>
