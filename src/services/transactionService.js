...
const updateParentStats = async (parentId) => {
  ...
  const colRef = collection(db, COLLECTION_PATH);
  // Find all children linked to this parent (Legacy + New Array)
  ...
  // FIX 1: Deduplicate children to prevent double-counting
  const uniqueChildrenMap = new Map();
  [...snap1.docs, ...snap2.docs].forEach(d => {
    uniqueChildrenMap.set(d.id, { id: d.id, ...d.data() });
  });
  ...
  if (!parentData.isReturn) {
    ...
    // Overpaid/Regular Settlement: I have this credit
  ...
  // ALWAYS reinitialize participantRemaining from splits
  ...
  // [PRESERVED ORIGINAL LOGIC] Initialize participantOverpaid bucket
  ...
  children.forEach(childData => {
    // FIX 2: ROBUST LINK AMOUNT LOGIC (Clamping)
    const link = childData.linkedTransactions?.find(l => l.id === parentId);

    // Prefer explicit link amount, fallback to child's full amount
    let rawLinkAmount = link ? parseFloat(link.amount) : parseFloat(childData.amount);
    if (isNaN(rawLinkAmount)) rawLinkAmount = parseFloat(childData.amount) || 0;

    // SANITY CHECK: If this is a simple 1-to-1 payment, clamp the link amount to the actual payment.
    // This fixes the issue where metadata says "597" but user paid "97".
    const childTotal = parseFloat(childData.amount) || 0;
    const isSingleLink = !childData.linkedTransactions || childData.linkedTransactions.length <= 1;

    if (isSingleLink && Math.abs(rawLinkAmount) > Math.abs(childTotal)) {
      // console.warn(`[Auto-Fix] Clamping link amount ${rawLinkAmount} to txn amount ${childTotal}`);
      rawLinkAmount = childTotal;
    }

    const linkAmount = Math.abs(rawLinkAmount);
    ...
    if (childData.isReturn) {
      ...
      // [PRESERVED ORIGINAL LOGIC]
      const isCreditBucket = startBalance < 0;
      ...
      // Child is an Expense -> INCREASE Debt (Add) / Consume Credit
      netBalance += linkAmount;
  ...
  const updateData = {
    ...
    participantStatuses,
    // [PRESERVED ORIGINAL LOGIC] Update overpaid amount if it tracks credit
    ...(parentData.isReturn ? {
